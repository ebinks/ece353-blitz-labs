...
---------------------------- InitFirstProcess -----------------------------
	function InitFirstProcess() 	--returns ptr to Thread
		var
			userProgram: ptr to Thread
		userProgram = threadManager.GetANewThread()
		(*userProgram).Init("User Program")
	...
	function StartUserProcess(i: int)
		...
		file = fileManager.Open("TestProgram4")
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
	...
	for i = 0 to MAX_FILES_PER_PROCESS-1 --  && fileDescriptorIndex == -1
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
	var
		virtPage, offset, chunkSize, nextPosInFile, copiedSoFar: int
	...
	while true
		...
		if virtPage < 0 || virtPage > currentThread.myProcess.addrSpace.numberOfPages - 1 || !currentThread.myProcess.addrSpace.IsWritable(virtPage) || !currentThread.myProcess.addrSpace.IsValid(virtPage)
			return -1  
		endIf
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
		var
			virtAddr, virtPage, offset, copiedSoFar, nextPosInFile: int
		...
		while true
			...
			if currentChunkSize <= 0
				break
			endIf
			if virtPage < 0 || virtPage > currentThread.myProcess.addrSpace.numberOfPages-1 || !currentThread.myProcess.addrSpace.IsValid(virtPage) || !currentThread.myProcess.addrSpace.IsWritable(virtPage)
				return -1 			
			endIf
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
	...
	if fileDesc < 0 || fileDesc >= MAX_FILES_PER_PROCESS || currentThread.myProcess.fileDescriptor[fileDesc] == null || newCurrentPos < -1 || newCurrentPos > currentThread.myProcess.fileDescriptor[fileDesc].fcb.sizeOfFileInBytes
